...
impl SqliteDb {
    ...
    pub fn read_from(&self, ordinal: Ordinal, allow_external: bool) -> Result<Vec<Event>, rusqlite::Error> {
        let mut stmt = self.con.prepare("SELECT * FROM events WHERE ordinal >= ?1 AND external >= ?2")?;
